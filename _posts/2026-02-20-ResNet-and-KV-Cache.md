...
ResNet 是沿深度展开的“非共享参数的残差迭代系统”
LSTM 是沿时间展开的“共享参数的递归动态系统”。
...
- 引入 Block AttnRes，将层划分为**压缩块**，使跨层注意力在规模上切实可行。利用注意力机制回头去评估前面所有层的产出，按需提取真正有用的信息。逐层回头看太费内存和通讯成本了 。所以作者设计了分块注意力残差（Block AttnRes），把网络切分成几个区块，后续的层只需要看前面区块的“浓缩总结”，完美解决了算力和内存的瓶颈。

...
![图1](/assets/image/20260220/2.png)
...
由此我们可以看到，如果以“途经点”监督的方式，其实 $H_i(x_i)$ 也是可以看作输入变量的，那么 $x_i$ -> $H_i(x_i)$ 的映射就有了新的理解，我们其实可以问，
...
空间换时间：用存储冗余来减少计算量（信息冗余）。
    
另外一类算法是计算冗余，所谓时间换空间：舍弃存储，用重复计算来换时间（计算冗余）。 
  
例如递归算法，回溯算法（Backtracking）和Fibonacci 流式生成器等。
